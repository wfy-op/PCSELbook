...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
      scale=0.87,
      layerlabel/.style={anchor=west, align=left, font=\scriptsize, text width=4.4cm}
    ]
    % Substrate at bottom
    \fill[gray!40] (-3,-5) rectangle (3,-4);
    \draw[thick] (-3,-4) -- (3,-4);
    \node[layerlabel] at (3.75,-4.55) {砷化镓衬底（$n$ 型掺杂）};
    ...
    \node[layerlabel] at (3.75,-3.55) {$n$ 型分布布拉格反射镜（20--30 对）};
    ...
    \node[layerlabel] at (3.75,-2.95) {下独立限制异质波导层};
    ...
    \node[layerlabel] at (3.75,-2.35) {有源区（3--5 阱多量子阱）};
    ...
    \node[layerlabel] at (3.75,-1.75) {上独立限制异质波导层};
    ...
    \node[layerlabel] at (3.75,-1.10) {光子晶体光栅层\\刻蚀深度~$150\,\mathrm{nm}$};
    ...
    \node[layerlabel] at (3.75,-0.62) {$p$ 型铝镓砷包层};
    ...
    \draw[thick] (-3,-0.3) -- (3,-0.3);
    \draw[thick] (-3,0) -- (3,0);
    \node[layerlabel] at (3.75,-0.20) {$p$ 型接触电极（钛/铂/金）};
    ...
    \draw[thick] (-3,0) -- (3,0);
    \draw[thick] (-3,0.3) -- (3,0.3);
    \node[layerlabel] at (3.75,0.25) {键合层（金-锡/$\mathrm{SiO_2}$）};
    ...
    \draw[thick] (-3,0.3) -- (3,0.3);
    \draw[thick] (-3,1) -- (3,1);
    \node[layerlabel] at (3.75,0.75) {热沉（铜/钨或金刚石）};
    ...
    \draw[thick] (3.38,-4) -- node[midway, right, rotate=90, font=\scriptsize, anchor=south] {外延总厚~$8$--$12\,\mu\mathrm{m}$} (3.38,0.3);
    ...
    \node[blue, align=center, font=\scriptsize, fill=white, fill opacity=0.82, text opacity=1, inner sep=1pt] at (0.95,-0.95) {基模场分布\\近高斯包络};
  \end{tikzpicture}
