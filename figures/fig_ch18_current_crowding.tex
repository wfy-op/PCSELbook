...
\begin{tikzpicture}[
  ...
  tick_label/.style={font=\footnotesize}
    ...
    \begin{scope}[shift={(0,0)}]
      ...
      \node[anchor=west, fill=white, inner sep=1pt, font=\scriptsize] at (5.65, 5.72) {$p$型接触边界};
      ...
      % Annotation arrows
      \draw[->, thick, red] (6.35, 5.20) -- (5.65, 5.15);
      \node[anchor=west, fill=white, inner sep=1pt, font=\scriptsize, red] at (6.25,5.45) {边缘电流密集};
      \draw[->, thick, blue] (6.35, 2.15) -- (4.0, 3.2);
      \node[anchor=west, fill=white, inner sep=1pt, font=\scriptsize, blue] at (6.25,2.35) {中心电流稀疏};
    ...
    \begin{scope}[shift={(7.8,2.2)}]
      ...
      % High/Low markers
      \node[anchor=west, font=\scriptsize, red] at (1.1, 4.15) {高 $J$};
      \node[anchor=west, font=\scriptsize, blue] at (1.1, -0.15) {低 $J$};
    \end{scope}
    
    % ========== Cross-section plot ==========
    \begin{scope}[shift={(9.65,0.65)}]
      \node[anchor=south, font=\large\bfseries] at (2, 3.35) {(b) 截面分布};
      
      % Axes
      \draw[->, thick] (0,0) -- (4.2,0) node[right, axis_label] {$x$ ($\mu$m)};
      \draw[->, thick] (0,0) -- (0,3.15) node[above, axis_label] {$J(x)$};
      ...
      % Contact edges
      \draw[dashed] (0.5, 0) -- (0.5, 3.0) node[above] {\scriptsize 边缘};
      \draw[dashed] (3.5, 0) -- (3.5, 3.0) node[above] {\scriptsize 边缘};
    ...
    \begin{scope}[shift={(0,-2.5)}]
      \draw[rounded corners, fill=orange!15] (-0.5,-1.48) rectangle (14.2, 0.45);
      \node[anchor=west, font=\footnotesize] at (-0.3, 0.1) {
        \textbf{物理机制}: 电流拥挤效应的成因
      };
      \node[anchor=west, font=\footnotesize] at (-0.3, -0.35) {
        • 横向电阻：空穴在p 型层中横向流动时遇到电阻，倾向于走最短路径（边缘）
      };
      \node[anchor=west, font=\footnotesize] at (-0.3, -0.80) {
        • 接触电阻：金属 - 半导体界面的非理想欧姆接触加剧边缘集中
      };
      \node[anchor=west, font=\footnotesize] at (-0.3, -1.20) {
