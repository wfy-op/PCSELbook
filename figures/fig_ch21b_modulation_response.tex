...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=1.0]
    ...
    \begin{scope}
      % Axes
      \draw[->, thick] (0,0) -- (7,0) node[right] {频率$f$ [GHz]};
      \draw[->, thick] (0,-4) -- (0,3) node[above] {归一化响应[dB]};
      ...
      \draw[<->, thick, magenta] (4.2,-3.3) -- node[below] {\scriptsize $f_{-3\mathrm{dB}}$} (5.5,-3.3);
    ...
    \begin{scope}[shift={(8,0)}, scale=0.8]
      % Axes
      \draw[->, thick] (0,0) -- (5,0) node[right] {时间 [UI]};
      ...
      \node[align=left, font=\footnotesize, anchor=north west] at (-0.5,-2.5) {
        \textbf{信号质量指标}:\\
        $\bullet$ 消光比 ER: $P_1/P_0 > 8\,\mathrm{dB}$\\
        $\bullet$ 抖动 Jitter: $<0.1\,\mathrm{UI}$\\
        $\bullet$ Q 因子:$>6$ (BER<$10^{-9}$)
      };
    \end{scope}
  \end{tikzpicture}
  \caption{PCSEL 的动态调制响应示意图。(a) 小信号频率响应随偏置电流提升而展宽；(b) 高速数字调制下的眼图及其常用信号质量指标。}
