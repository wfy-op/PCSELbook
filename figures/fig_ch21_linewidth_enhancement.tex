...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=1.0]
    ...
    \begin{scope}[shift={(2,0)}]
      ...
      \draw[thick, blue, domain=-3:3, samples=100] 
        plot(\x,{2/(1+\x*\x)});
      \node[above, blue] at (0,2.2) {ST 线宽$\Delta\nu_{\mathrm{ST}}$};
    ...
    \node[draw, align=center, font=\small] at (0,-2.5) {
      ...
      物理意义：振幅 - 相位耦合强度\\
      典型值：体材料$\alpha_H\sim 3$--$6$, QW $\alpha_H\sim 2$--$4$
    };
  ...
  \end{tikzpicture}
  \caption{Henry 线宽增强因子 $\alpha_H$ 的物理来源及其对 PCSEL 光谱纯度的影响。(a) 左图示意载流子变化同时引起增益和折射率变化，从而产生振幅与相位耦合；(b) 右图示意 $\alpha_H$ 使实际线宽相对 Schawlow-Townes 极限进一步展宽，并在动态调制中引入频率啁啾。}
