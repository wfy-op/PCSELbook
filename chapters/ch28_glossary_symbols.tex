...
本章先给出核心术语表，再给出符号总表，随后整理最常见的混淆对照，最后说明本章在工作流、例题与失败模式诊断中的使用位置。
...
\input{figures/fig_ch28_symbol_glossary}
\begin{IntuitionBox}
术语表不是为了把词翻译得“直观”，而是为了让证据链不失真。PCSEL 研究跨越开放电磁系统、半导体器件和多物理场建模，同一个词若在不同章节里含义漂移，结论就会在不知不觉中越级。
\end{IntuitionBox}
...
\cref{ch:workflow} 负责说明建模链应该怎样向前推进；\cref{ch:worked-example} 完整展开这条链；\cref{ch:failure-modes} 则把同一条链倒过来，检查哪里最容易断。本章的作用，是确保这三条阅读路径使用的是同一套词汇和符号。
...
\section*{本章小结}
本章把全书中最容易漂移的术语和符号压缩成一份统一工具表。有效的用法，不是偶尔查一个词，而是在写公式、做工作流、整理例题中间结果和诊断失败模式时，持续用它检查自己是否发生了层级混用、概念越级或符号漂移。
...
\section*{建议继续阅读}
\begin{itemize}
  \item 若你在整理理论与辐射主线，建议与 \cref{ch:bic,ch:math-appendix} 对照阅读。 阅读时应同时核对本章变量在后续模型中的定义和适用范围。

  \item 若你在整理调制、线宽与噪声主线，建议与 \cref{ch:dynamics,ch:device-appendix} 对照阅读。 阅读时应同时核对本章变量在后续模型中的定义和适用范围。

  \item 若你在整理项目执行链，建议与 \cref{ch:workflow,ch:worked-example,ch:failure-modes,ch:sim-appendix} 对照阅读。 阅读时应同时核对本章变量在后续模型中的定义和适用范围。
\end{itemize}
