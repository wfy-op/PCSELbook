...
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[node distance=1.45cm,>=Stealth]
    \node[draw,rounded corners,fill=blue!10,minimum width=2.8cm] (e) {Electrical solve};
    \node[draw,rounded corners,fill=red!10,right=of e,minimum width=2.8cm] (h) {Thermal solve};
    \node[draw,rounded corners,fill=green!10,right=of h,minimum width=2.8cm] (o) {Optical solve};
    \draw[->] (e)--node[above]{\small $\bm{J},R,Q$}(h);
    \draw[->] (h)--node[above]{\small $T(\rvec)$}(o);
    \draw[->,dashed,bend left=18] (o) to node[below]{\small $\alpha,g,n$ feedback}(e);
  \end{tikzpicture}
  \caption{热-电-光耦合流程图。真正的器件工作点通常需要通过这种闭环迭代获得，而不是单次串行后处理。}
  \label{fig:eto-flow}
\end{figure}
