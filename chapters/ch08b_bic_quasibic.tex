...
如果设计目标只是追求无穷大的被动$Q$，理想 BIC看起来像终点；但 PCSEL不是被动共振器展示题，它要作为激光器工作。激光器需要模式既有足够低的辐射损耗，以降低阈值增益，又保留可控的垂直辐射，以便输出、整形和利用远场。因此，真实器件更常瞄准的是\textbf{靠近BIC 的quasi-BIC 区域}。
...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_ch08b_bic_mechanism.tex}
  \caption{BIC 形成机制示意图。(a) 对称保护 BIC；(b) 干涉型 BIC；(c) quasi-BIC 区域及其 $Q\propto\delta^{-2}$ 标度。}
  \label{fig:ch08b_bic_mechanism}
\end{figure}

\cref{fig:ch08b_bic_mechanism}揭示了一个深刻的物理事实：BIC并非某种神秘的例外状态，而是波动系统在对称性或干涉条件下自然涌现的本征态。对于PCSEL设计者而言，关键不在于"是否利用BIC"，而在于如何将quasi-BIC区域的超高Q特性与激光器的增益、阈值和远场需求协同优化。
...
\end{equation}
其中 $d$ 是破缺位移或等效几何扰动幅度，$a$ 是晶格常数，$Q_0$ 吸收了结构 family、工作带支和归一化定义相关的常数。\cref{eq:q_delta_explicit} 的直接后果是：若其余条件近似不变，把 $\delta$ 从 $0.02$ 降到 $0.01$，辐射 $Q$ 通常会提升约四倍。该二次标度在对称破缺 quasi-BIC 文献中有明确支撑\cite{koshelev2018quasibic}。它告诉读者，轻微几何扰动并不是“把高 $Q$ 模一下子彻底毁掉”，而常常是在用一个可控旋钮平滑地打开辐射通道。于是设计问题就变成：给定目标输出功率、阈值增益预算、偏振纯度和远场要求，应该把 $\delta$ 选在什么区间，而不是机械追求越接近 BIC 越好。
...
除“单点附近的 quasi-BIC”外，近年还出现了 quasi-bound flat bands in the continuum（quasi-BFIC）路线：其目标不是只在单一动量点获得高 $Q$，而是在更宽的角度/动量区间维持较高 $Q$ 与可用辐射耦合。对 PCSEL 设计的意义是两点：第一，工艺与角度容差可提高；第二，有限尺寸阵列中的角谱展宽不再必然迅速拉低整体辐射品质。该结论在 2025 年直接工作中已有证据支撑\cite{qin2025quasibfic}。  
但 quasi-BFIC 不是“替代”传统 $\Gamma$ 点 quasi-BIC，而是补充：前者更强调宽角鲁棒，后者更强调中心点高选择性。工程上应根据目标（窄主瓣极限、容差、功率提取）选择路线；BIC/quasi-BIC 综述与标度工作更适合作为背景理论锚点\cite{hsu2016,koshelev2018quasibic}。
