...
若进一步用群速度 $v_g$ 把空间增益换成时间增益率，则
\begin{equation}
\gamma_{\mathrm{opt}} = v_g g_{\mathrm{mod}} = v_g\Gamma_{\mathrm{opt}} g_{\mathrm{mat}}.
\label{eq:gamma_opt}
\end{equation}
同理，若把辐射损耗和内部损耗写成沿传播方向的等效损耗系数，则
\begin{equation}
\gamma_{\mathrm{rad}}=v_g\alpha_{\mathrm{rad}},
\qquad
\gamma_{\mathrm{int}}=v_g\alpha_{\mathrm{int}}.
...
\begin{table}[htbp]
  \centering
  \small
  \caption{本章阈值链中最容易混淆的量纲桥。若不先把这一层单位统一，$v_g g$ 与 $\alpha/\Gamma$ 的映射最容易被读错。}
  ...
  \begin{tabular}{lll}
    ...
    $v_g$ & $\mathrm{cm/s}$ 或 $\mathrm{m/s}$ & 把空间增益/损耗映射成时间率的等效参数 \\
    \bottomrule
  \end{tabular}
\end{table}

\begin{RigorousBox}
这里的 $v_g$ 必须读成\textbf{把空间增益/损耗映射成时间增长率的等效参数}，而不能机械地理解成”PWEM 带图在 exact $\Gamma$ 点处的 Bloch 群速度”。对 PCSEL 的 standing-wave / 区中心模式来说，带图斜率趋零与速率方程里仍保留一个有限的等效增益转换参数，并不矛盾，因为两者服务的是不同降阶层级：前者描述 Bloch 包络的传播速度（宏观驻波），后者描述局域光场与增益介质的耦合速率（微观增益-场相互作用）。实际建模时，常把 $v_g$ 估算为 $c/n_{\mathrm{eff}}$，其中 $n_{\mathrm{eff}}$ 为纵向等效折射率；对 GaAs 基 PCSEL 而言，$n_{\mathrm{eff}}\approx3.3$--$3.5$，因此 $v_g\approx0.9\times10^{10}\,\mathrm{cm/s}$ 可作为数量级参考，但最终应与速率方程拟合或传播矩阵法对齐。
\end{RigorousBox}
...
\end{equation}
这里 $\alpha_{\mathrm{FCA}}$ 表示自由载流子吸收，$\alpha_{\mathrm{abs}}$ 表示材料吸收或掺杂层吸收，$\alpha_{\mathrm{metal}}$ 表示金属邻近引入的损耗，$\alpha_{\mathrm{scatt}}$ 表示与粗糙度、侧壁和工艺缺陷相关的散射损耗。对典型 GaAs 基 PCSEL 外延结构，各项量级大致为：$\alpha_{\mathrm{FCA}}\sim1$--$10\ \mathrm{cm}^{-1}$（随掺杂浓度线性增大），$\alpha_{\mathrm{abs}}\sim1$--$5\ \mathrm{cm}^{-1}$，$\alpha_{\mathrm{metal}}$ 若模式尾场触及欧姆接触层可达 $10$--$100\ \mathrm{cm}^{-1}$，$\alpha_{\mathrm{scatt}}$ 通常 $<5\ \mathrm{cm}^{-1}$。辐射损耗 $\alpha_{\mathrm{rad}}$ 对高-$Q$ BIC 候选模可低至 $0.1\ \mathrm{cm}^{-1}$，对亮模可达 $10\ \mathrm{cm}^{-1}$ 以上（仅供数量级参考）。
