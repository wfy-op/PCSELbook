...
\label{eq:s_from_a}
\end{equation}
于是只有当 $\gamma_{\mathrm{net}}>0$ 时，模式才会衰减；当 $\gamma_{\mathrm{net}}<0$ 时，线性模型会预言指数增长；而阈值点恰好是这两类行为的分界面。换句话说，\cref{eq:threshold_gamma} 的地位是“把增长和衰减的边界精确写出来”，而非“把一个符号定义成零”。
...
\begin{WarningBox}[能量权重近似的适用条件]
式~\eqref{eq:gamma_opt_def}的能量权重形式依赖以下近似：
\begin{enumerate}
  \item \textbf{缓变包络近似：}模式剖面在增益区域内变化缓慢，可用单一模式场 $\E(\rvec)$ 描述
  \item \textbf{标量增益近似：}材料增益在有源区内可视为局域标量，忽略各向异性和空间非均匀性
  \item \textbf{小信号近似：}增益饱和效应不显著改变模式剖面形状
  \item \textbf{单模近似：}忽略多模竞争和空间烧孔效应对有效 $\Gamma_{\mathrm{opt}}$ 的影响
\end{enumerate}
对高功率 PCSEL 或存在强空间烧孔的情况，有效约束因子可能偏离式~\eqref{eq:gamma_opt_def}的预测，需通过多模自洽求解获得准确的 $\Gamma_{\mathrm{opt}}^{\mathrm{eff}}$。
\end{WarningBox}
这里的写法刻意保持在“最小接口层”。但它不是无条件成立的定义，而依赖几条近似：材料增益在有源区内可先看作局域标量；目标模式剖面在小信号增益下不发生显著重构；极化选择和增益色散已被压缩成该模式实际看到的一条有效增益曲线。若要把 $g_{\mathrm{mat}}$ 写成透明载流子密度、微分增益、量子阱数目与温度失谐的函数，要等到 \cref{ch:qw-gain} 才系统展开；本章先把它当作一个尚未完全展开、但已经明确属于有源区模型的输入量。
...
更严格的写法可从能量速度而不是带图群速度出发。设模式总储能为
$U$，有源区提供的净增益功率为 $P_{\mathrm{gain}}$，则定义时间增长率的直接方式是
\begin{equation}
...
这条式子需要按层级理解：它给出的首先是\textbf{光学线性阈值}，不是器件级阈值电流。
...
\end{equation}
这里 $\alpha_{\mathrm{FCA}}$ 表示自由载流子吸收，$\alpha_{\mathrm{abs}}$ 表示材料吸收或掺杂层吸收，$\alpha_{\mathrm{metal}}$ 表示金属邻近引入的损耗，$\alpha_{\mathrm{scatt}}$ 表示与粗糙度、侧壁和工艺缺陷相关的散射损耗。\cref{fig:ch07_loss_breakdown_piechart}以饼图形式直观展示了典型 PCSEL 器件中各项损耗的占比情况。为避免把经验值误读为“标准常数”，本书统一采用“范围 + 条件 + 来源”写法：
\textbf{[文献值]} 对近红外 GaAs 基 PCSEL/半导体激光器的代表性外延与掺杂条件，常见有
$\alpha_{\mathrm{FCA}}\sim1$--$10\ \mathrm{cm}^{-1}$（随掺杂与载流子密度显著变化）、
$\alpha_{\mathrm{abs}}\sim1$--$5\ \mathrm{cm}^{-1}$、
$\alpha_{\mathrm{metal}}\sim10$--$100\ \mathrm{cm}^{-1}$（当模式尾场靠近金属层时）、
$\alpha_{\mathrm{scatt}}\lesssim5\ \mathrm{cm}^{-1}$\cite{coldren2012,zhoupan2023}；
\textbf{[本书算例]} 辐射损耗 $\alpha_{\mathrm{rad}}$ 在高-$Q$ 候选模可低至 $10^{-1}\ \mathrm{cm}^{-1}$ 量级，在亮模可达 $10\ \mathrm{cm}^{-1}$ 以上，具体数值必须与结构、边界与提取方法一并报告。

\input{figures/fig_ch07_loss_breakdown_piechart}
...
\cref{fig:ch07_q_threshold_decision_chain} 把这个误区拆成一条更稳妥的判断链。高 $Q$ 首先只影响辐射损耗入口；它要经过内部损耗、约束因子、注入效率和热回写之后，才可能变成低阈值电流和稳定工作窗口。

\begin{figure}[htbp]
  \centering
  \input{figures/fig_ch07_q_threshold_decision_chain}
  \caption{从被动高 $Q$ 到阈值和工作窗口的层级链。高 $Q$ 有助于降低辐射损耗，但总阈值还要同时检查内部损耗、模场与有源区重叠、注入效率和热回写后的模式排序。}
  \label{fig:ch07_q_threshold_decision_chain}
\end{figure}

\section{从速率方程再看一遍阈值}
若采用单模近似，光子数 $S$ 的最简速率方程可写为
\begin{equation}
\frac{\dd S}{\dd t} = v_g\bigl[\Gamma_{\mathrm{opt}} g_{\mathrm{mat}}(N,T)-\alpha_{\mathrm{rad}}-\alpha_{\mathrm{int}}\bigr]S + \beta_{\mathrm{sp}}R_{\mathrm{sp}} V_{\mathrm{mode}}.
\label{eq:photon_rate}
\end{equation}
其中 $N$ 为等效载流子浓度，$T$ 为温度，$\beta_{\mathrm{sp}}R_{\mathrm{sp}} V_{\mathrm{mode}}$ 表示耦入该模式的自发辐射项，$V_{\mathrm{mode}}$ 为模式体积。

\begin{notebox}
\textbf{速率方程的量纲一致性。}式\cref{eq:photon_rate}中各项必须具有相同量纲：
\begin{itemize}
  \item $S$ 是光子数（无量纲计数），故 $\dd S/\dd t$ 的单位是 $\mathrm{s}^{-1}$
  \item $v_g[\cdots]S$ 项：$v_g$ (m/s) $\times$ $\alpha$ (m$^{-1}$) $\times$ $S$ $\rightarrow$ $\mathrm{s}^{-1}$ $\checkmark$
  \item $\beta_{\mathrm{sp}}R_{\mathrm{sp}} V_{\mathrm{mode}}$项：$\beta_{\mathrm{sp}}$ (无量纲) $\times$ $R_{\mathrm{sp}}$ (m$^{-3}$s$^{-1}$) $\times$ $V_{\mathrm{mode}}$ (m$^3$) $\rightarrow$ $\mathrm{s}^{-1}$ $\checkmark$
\end{itemize}
若省略 $V_{\mathrm{mode}}$，则自发辐射项的量纲将是 m$^{-3}$s$^{-1}$，与光子数变化率的量纲不匹配。
\end{notebox}
...
\end{equation}
因此，从复频率角度看，阈值条件并非另一条新物理，而只是把“净衰减归零”重写成“复频率虚部穿过零”的开放系统表达。它和 \cref{eq:threshold_gamma,eq:threshold_rate} 必须给出同一个阈值。
...
\cref{eq:gth,eq:threshold_rate} 与后文 CWT 的 `max Re$(\lambda_j)=0$` 并非两套
...
\cref{eq:gmod_nonuniform} 的意义非常深刻。它说明：即便两个模式拥有相同的冷腔损耗，若它们在横向上的场分布不同，而注入分布又不均匀，那么它们得到的有效模增益也会不同。这也是后面电注入和热效应能够改写模式排序的根本原因。
...
因此，本章故意把问题停在 $g_{\mathrm{th}}$ 与阈值电流来源的分界线上。这样做是为了确保读者先把静态层级分清，再进入后面的 \cref{ch:dynamics} 去讨论弛豫振荡、调制响应和线宽来源，而非回避动态学。若在这里就把阈值、稳定性和线宽混写在一起，最容易造成的错误恰恰是重新把“能起振”误当成“能稳定单模工作”。
...
第一，\textbf{对 PCSEL 入门与器件设计最先起决定作用的，通常是模式候选、损耗排序、注入分布和热反馈的主导关系，而非完整阈上自洽解。} 这些问题若没先理清，即使直接写出更高层形式化，也很难得到可操作的设计判断。
...
\input{figures/fig_ch07_threshold_gain_breakdown}

\subsection{从注入电流到阈值条件：为什么需要一张 Sankey 图}
公式链条
\[
I_{\mathrm{inj}}\rightarrow N(\rvec)\rightarrow g_{\mathrm{mat}}\rightarrow g_{\mathrm{mod}}\rightarrow g_{\mathrm{th}}
\]
在文字里看起来很线性，但实际每一跳都会“损耗/折算”一部分可用增益。为把这种逐级折算直观化，\cref{fig:ch07_threshold_current_sankey} 给出一张教学型 Sankey 示意图。它不追求某个器件的精确百分比，而是强调：阈值电流上升往往是多个环节的小损失叠加而成，而非由某一项单独造成。

\input{figures/fig_ch07_threshold_current_sankey}

这张图和本章公式可一一对上：注入侧折算对应 \cref{eq:gmod_nonuniform} 的空间非均匀项；复合侧折算对应“材料增益不是全都转化为受激增益”；模场侧折算对应 \cref{eq:gamma_opt_def} 中的约束因子；最终只剩能对冲 \cref{eq:threshold_g_th_final} 右端总损耗的那一部分。对工程设计而言，这比单独报告一个 $g_{\mathrm{th}}$ 数字更有诊断价值，因为它直接告诉你下一步该优先优化“注入”“重叠”还是“损耗”。

\section*{回看本章}
本章把阈值问题严格放回了它应在的层级：阈值首先是净模增益补偿总损耗的光学条件。材料增益必须通过约束因子与模式重叠转化为模增益；总损耗则必须拆成辐射与内部各项来源。只有把这些关系讲清楚，后面讨论阈值电流、电注入和热效应才不会偷换概念。与此同时，辐射损耗这一项并非孤立参数：在 \cref{ch:bic} 中，读者会看到同一类 $\Gamma$ 点辐射通道如何被 BIC / quasi-BIC 语言重新组织；而在 \cref{ch:dynamics} 中，读者会看到“谁先达到阈值”还必须继续延伸为“阈上谁更稳定、谁的调制更快、谁的线宽更窄”。这也是 PCSEL 中静态阈值、辐射工程与动态学不可拆开的原因。
...
\section*{延伸阅读}
\begin{itemize}
  \item \cite{coldren2012,siegman1986,henry1982} 中关于阈值条件、速率方程与线宽来源的经典章节和论文。

  \item 读完 \cref{ch:qw-gain,ch:electrical} 后回看本章，会更清楚“阈值增益”与“阈值电流”之间相隔哪些物理环节。

  \item 若你想继续追问“辐射损耗本身是怎样被结构对称性重排的”，可接着读 \cref{ch:bic}；若你想追问“达到阈值以后会怎样演化”，则应直接进入 \cref{ch:dynamics}。若你还想知道这条链在更高层有源自洽理论中的位置，可把 \cite{tureci2007salt,tureci2009salt} 作为背景参考，而不必把它当成本书主线的前置条件。
\end{itemize}
