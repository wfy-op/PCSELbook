...
\end{equation}
其中 $k_0=\omega/c$。\textbf{[文献值]}对 GaAs 量子阱，$\alpha_H$ 常见于 $2$--$5$ 区间\cite{henry1982,coldren2012}，该典型值主要对应单模行波半导体激光器语境；迁移到 PCSEL 时必须按模式与工作点重新标定。
...
\end{itemize}
因此，对 PCSEL 而言，$\alpha_H=2$--$5$ 仅能作为数量级参考。精确的线宽预测需要使用基于具体模式结构和噪声源的微观模型。
\end{warningbox}

在行波近似下，线宽常相对理想 Schawlow--Townes 极限再乘上 $(1+\alpha_H^2)$ 的放大量级。该估计有助于建立趋势判断，但在 PCSEL 中不能机械套用，原因有两点。第一，$\alpha_H$ 的标准推导基于单模行波激光器中振幅扰动通过载流子变化回写到相位这一图像；PCSEL 工作在驻波/区中心模式，相位 - 幅度耦合的空间结构与行波激光器不同，标准单模 $\alpha_H$ 不能直接描述多波耦合模的相位响应。第二，PCSEL 多模竞争、偏振切换和热漂移会引入额外低频相位调制，这些贡献不包含在 $\alpha_H$ 的定义中。因此，一旦工作点接近偏振切换、模式跳变或强热漂移，以单模平稳近似为基础的 $(1+\alpha_H^2)$ 线宽放大公式就会失效。
为了帮助读者直观理解线宽增强因子的物理来源及其对光谱纯度的影响，\cref{fig:ch21_linewidth_enhancement}展示了 Henry 因子的定义及其导致的线宽展宽机理。右图中“$\alpha_H=3$ 时约放大 10 倍”应读作\textbf{[教学示意]}数量级，不是所有 PCSEL 的固定倍数。

\paragraph{何时必须从单模 Henry 模型升级}
当出现以下任一情形时，应从单模 Henry 公式升级到多模/非正交或系统级噪声模型：  
(i) 主模与边模净裕量接近，存在模式跳变；  
(ii) 偏振分支在工作窗口内可切换；  
(iii) 温升回写显著改变模排序；  
(iv) 频率调制与相位噪声耦合成为系统指标。  
这在高亮度 PCSEL 与调制型 PCSEL 报道中尤为常见\cite{suh2004tcmt,itoh2020,yoshida2023}。
...
\end{equation}
估计总半高全宽\cite{coldren2012}。\cref{eq:lorentz_from_freqnoise,eq:gaussian_linewidth,eq:voigt_width} 的真正价值，不是要求读者把数字系数背下来，而是帮助读者建立一条清楚的诊断链：\textbf{先看频率噪声来自快白噪声还是慢漂移，再决定线宽应按 Lorentz、Gaussian 还是 Voigt 去解释。}
...
因此，在 PCSEL 的研究报告中，合理做法不是孤立汇报“线宽”两个字，而是同时给出观测时间窗、偏振条件、工作电流和温度。这样，线宽才能与相干性和模式稳定性建立明确对应。
...
\begin{RigorousBox}
Noda 等人的 2023 tutorial 给了一个很好的性能报告反例库：同样是“高亮度 PCSEL”，\SI{500}{\micro\meter} 双晶格器件可在脉冲条件下达到 \SI{10}{W} 级、约 $0.2^\circ$--$0.3^\circ$ 发散；引入背面 DBR 后，脉冲峰值功率超过 \SI{20}{W}，亮度达到约 \(1.5\,\mathrm{GW\,cm^{-2}\,sr^{-1}}\)；\SI{1}{mm} 器件在 CW 下达到 \SI{10}{W} 级，并通过频率噪声白噪声平台给出低于 \SI{1.23}{kHz} 的内禀线宽估计；\SI{3}{mm} 器件进一步实现 \SI{50}{W} CW、约 $0.05^\circ$ 发散和 \(1\,\mathrm{GW\,cm^{-2}\,sr^{-1}}\) 亮度\cite{noda2023tutorial,yoshida2019,yoshida2023}。这些数字必须和口径、脉冲/CW、热边界、线宽测量带宽、远场定义一起读。
...
\cref{tab:noda2023_metric_protocol} 的作用不是替读者背诵性能纪录，而是训练一种读文献的顺序：先看工作条件，再看指标定义，最后才比较数字。若忽略这一步，\SI{50}{W} 连续波、\SI{20}{W} 脉冲峰值、\SI{1.23}{kHz} 内禀线宽估计和 $M^2\approx2.36$ 很容易被错误地并排解读；实际上，它们分别回答的是功率可扩展性、辐射提取、时间相干和空间光束质量的不同问题。
...
\section*{本章小结}
PCSEL 的性能指标必须在工作点上下文中理解。线宽来自谱形定义并依赖观测窗口，相干性既有时间侧也有空间侧，$M^2$ 和亮度要求把功率与发散一起看，偏振纯度和远场稳定性则是模式稳定性的外部投影。频率噪声谱、短时/长时线宽、Petermann 因子和外腔反馈进一步提醒我们：开放腔激光器的“性能数字”总是带着测量协议和环境边界条件。在建模层面，Lumerical 与 COMSOL 都擅长直接计算模式、辐射、载流子和温度等确定性中间量，但线宽和相干性通常需要借助这些中间量进入降阶噪声模型间接估计。可靠的性能报告，不是“报一个最直观的数字”，而是报告一个可复核的工作窗口。
