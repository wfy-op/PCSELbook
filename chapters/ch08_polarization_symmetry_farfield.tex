...
如果只停留在“镜面对称、旋转对称、奇偶性”这些词上，读者虽然能听懂很多物理解释，但还没有真正得到一套可以反复使用的分类工具。真正教材化的做法，是在 $\Gamma$ 点把模式按小群（little group）的不可约表示分类。
...
\end{equation}
其中 $\mathcal{G}$ 是晶格的点群。对 $\Gamma$ 点而言，$\kvec=\bm{0}$，因此
\begin{equation}
\mathcal{G}_{\Gamma}=\mathcal{G}.
\end{equation}
这就是为什么 $\Gamma$ 点最适合做正式对称性标注：此时小群就是完整点群，模式标签最清楚、选择规则也最严格。

不可约表示（irreducible representation, irrep）的作用可以概括成三条：
\begin{enumerate}
  \item 同一 irrep 内的模式在保持该对称性的扰动下可以混合，不同 irrep 的模式则不能直接混合；
  \item 二维 irrep 对应受对称性保护的简并，因此天然与双偏振、双简并候选模有关；
  \item 模式是否能耦合到法向辐射通道，首先取决于它的 irrep 是否与辐射通道的 irrep 匹配。
\end{enumerate}

物理根源：在 $\Gamma$ 点（$k_\parallel=0$），法向辐射对应的自由空间平面波是均匀平面波，其横向线偏振电场在 $C_{4v}$ 语言中变换为 $E$ 表示。若腔模的 irrep 与自由空间辐射场的 irrep \textbf{不匹配}（即它们在所有对称操作下的特征标乘积积分为零），则两者之间的辐射耦合矩阵元为零——这正是 $A_1$、$A_2$、$B_1$、$B_2$ 四类非简并模通常对应暗模（BIC）的群论原因。只有 $E$ 表示（二维简并模）可与线偏振辐射耦合，是典型的亮模。

\begin{RigorousBox}
本章的 irrep 分类是“最小但可用”的工具。它主要处理平面内 little group 与法向辐射选择规则，已经足以支撑 PCSEL 中的偏振选择、$\Gamma$ 点模标注和 symmetry-protected BIC 判断。若要得到完全严格的三维 slab 结论，还需再结合纵向奇偶性和真实层栈不对称。
\end{RigorousBox}
