...
\end{itemize}
这就是为什么高阶带边模式（通常是 $E$ 表示）能够表面发射，而基模（通常是 $A_1$）却可能是暗模的根本原因。
...
例如，若近场在 $x$ 方向关于中心是奇对称的，那么在 $k_x=0$ 附近，来自左右两侧的贡献会相互抵消，远场中心就可能出现零点或主瓣分裂。反之，若近场是偶对称的，则中心角附近更容易形成主瓣。这种"对称性 $\rightarrow$ 远场中心结构"的映射，是 PCSEL 远场整形的核心直觉。\cref{fig:ch08_farfield_pattern}展示了两种典型模式的远场辐射方向图对比。
...
对 PCSEL而言，第二条路径尤其常见，因为偏振本身就与辐射选择规则紧密相关。于是很多偏振工程本质上不是在"转动偏振方向"，而是在"重新安排不同偏振态与辐射/损耗通道的相对权重"。
...
\cref{fig:ch08_four_mode_triplet}的物理含义极其重要：它揭示了PCSEL 模式结构的层级组织原则——首先是行波到驻波的重组，然后是 irrep层面的对称性分类，最后是微扰导致的剩余简并解除。这一图像是理解后续偏振选择和模式分裂现象的基础。
