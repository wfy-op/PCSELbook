...
\begin{notebox}
\textbf{纵向传播常数矩阵 $\mathbf{\beta}_\ell$ 的定义。}对第 $\ell$ 层的第 $n$ 个空间谐波，其纵向传播常数为
\[
\beta_{\ell,n} = \sqrt{k_0^2\varepsilon_{\ell,n} - |\bm{k}_{\parallel,n}|^2},
\]
其中 $\varepsilon_{\ell,n}$ 是该层第 $n$ 个 Fourier 分量的相对介电常数，$\bm{k}_{\parallel,n} = \bm{k}_{\parallel,0} + \bm{G}_n$ 是第 $n$ 个面内波矢。对所有保留的空间谐波，$\mathbf{\beta}_\ell$ 是一个对角矩阵，其对角元为各个谐波的 $\beta_{\ell,n}$。当 $\beta_{\ell,n}$ 为纯虚数时，对应倏逝波；当为实数时，对应传播波。
\end{notebox}
