...
\cref{fig:ch13_rcwa_discretization}揭示的离散化思想是RCWA能够处理复杂周期结构的关键：任何连续的剖面都可以用足够多的薄层来逼近，而每一层内部的平面内周期性使得我们可以用傅里叶级数展开。这种"分层+展开"的策略，正是 RCWA既能保持半解析方法的精度，又能处理任意复杂剖面的原因所在。
...
\begin{itemize}
  ...
  \item 边界：当问题进入强非线性增益、强有限尺寸边缘效应或多物理场强耦合时，仍需回到全波与器件级模型完成闭环。
\end{itemize}
