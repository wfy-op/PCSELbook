...
换句话说，阻挡层的器件意义不是改善表观电流利用率，而是避免本章的材料增益在映射到 \cref{ch:electrical} 的注入分布时被严重稀释。
