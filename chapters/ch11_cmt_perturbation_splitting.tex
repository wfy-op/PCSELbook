...
\end{equation}
在封闭系统中，这导致耦合矩阵满足 $\kappa_{12} = \kappa_{21}^\ast$ (厄米性)。
...
\textbf{开放系统中的"表观非互易":} 即使介质本身满足互易性，开放系统的辐射通道也可诱导\textbf{有效非互易耦合} $\kappa_{12} \neq \kappa_{21}$。这是因为：
\begin{itemize}
...
\end{itemize}
这种"表观非互易"并不违反 Lorentz 定理，而是反映了开放系统动力学的固有特性：辐射通道充当了"环境"，介入了模式间的有效相互作用。
...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_ch11_cwt_cmt_mapping.tex}
...
\cref{fig:ch11_cwt_cmt_mapping}的实际用途非常明确：当你需要解释"为什么改变孔形会影响偏振"时，用CWT追踪散射通道的变化；当你需要解释"为什么改变金属厚度会影响阈值"时，用TCMT追踪损耗通道的变化；当你需要同时考虑两者时，就用映射关系把它们串起来。
...
这三者之间最合理的关系不是并列选择，而是层层递进。常见的工作路径是：先用CWT找到主导模和主导耦合；再在该模附近用微扰公式快速评估若干小改动；最后把得到的候选参数区间送入TCMT 或全波散射分析，检查端口响应和实际输出行为。这种串联方式往往比"从一开始就完全黑箱扫参"更高效，也更容易形成可迁移的直觉。
