...
在 TCMT 语言中，前者更像共振模之间的内部耦合矩阵 $\bm{\Omega}$，后者更像通过共同端口诱导的耗散矩阵 $\bm{\Gamma}_{\mathrm{rad}}$。在微扰语言中，双晶格间距、孔径比例、背面 DBR 相位和 p 包层厚度都不是单纯“调频率”的旋钮，而是在同时改变复频率的实部与虚部。若只追踪频率分裂而不追踪辐射常数曲率，就会漏掉大面积单模设计真正依赖的损耗裕量。
