...
若只想先得到一个最小横向扩展模型，可把 current spreading layer 近似成一张片电阻为 $R_{\square}$ 的二维导电片，其片上电位 $V_s(x,y)$ 满足
\begin{equation}
\nabla_\parallel^2 V_s-\frac{V_s-V_{\mathrm{cont}}}{L_s^2}=0,
\label{eq:current_spreading_sheet}
\end{equation}
其中 $V_{\mathrm{cont}}$ 是接触电位，$L_s$ 是电流扩展长度。\cref{eq:current_spreading_sheet} 当然不能替代完整漂移--扩散求解，但它在教学上非常有价值：只要 $L_s$ 明显小于器件横向尺寸，均匀注入就不再是默认可信假设。
...
\label{eq:cont_p}
\end{align}
其中 $R$ 表示总复合率，$G$ 表示产生项。对电注入 PCSEL 来说，$G$ 主要体现在边界与接触的注入通量中。\cref{eq:jn_dd,eq:jp_dd,eq:cont_n,eq:cont_p} 都属于\textbf{电学模型}。
