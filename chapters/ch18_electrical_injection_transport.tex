...
\end{equation}
其中 $\eta_{\mathrm{inj}}$ 是有效注入效率，$I_{\mathrm{leak}}$ 汇总漏电和旁路电流。对典型 PCSEL 结构，$\eta_{\mathrm{inj}}$ 常先按 $0.6$--$0.9$ 做\textbf{[本书算例]}敏感度扫描，再由具体结深、阻挡层和 spreading layer 设计收紧。\cref{eq:ith_from_nth} 的核心意义不是给出单一高保真数值，而是把“同一个 $N_{\mathrm{th}}$ 为何会对应完全不同的 $I_{\mathrm{th}}$”写成公式：只要复合通道更强、注入效率更低或漏电更大，阈值电流就会明显抬升。

\section{把局域载流子分布重新接回模增益：模式竞争在这里被重写}
当前面所有电学求解完成后，本章输出给光学模型的不是一个总电流值，而是量子阱中的局域载流子分布 $N(x,y,z)$。对于候选模式 $m$，更一般的有效模增益应写成
\begin{equation}
 g_{m,\mathrm{eff}}
 =
...
\section*{本章小结}
电注入 PCSEL 的阈值电流问题，本质上是在问：有限接触和有限横向导电能力下，外部电流怎样通过接触边界、current spreading layer、异质结、复合与漏电，转化为与目标模式匹配的局域载流子分布。欧姆/肖特基接触通过边界条件进入模型，接触电阻和接触几何重塑横向掉压，电流孔径与 spreading length 决定注入是否接近均匀；Poisson+漂移-扩散+连续性方程给出了器件电学骨架，而有效模增益必须通过局域载流子分布重新加权。到本章结束时，读者应当已经完全清楚：阈值电流不是阈值增益的换单位版本，而是一个空间分辨的器件工作点问题。
