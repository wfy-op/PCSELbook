...
为了帮助读者直观理解 PCSEL 的小信号调制响应特性，\cref{fig:ch21b_modulation_response}展示了不同偏置电流条件下的典型调制响应曲线。图中包含了三条曲线，分别对应$I/I_{th}=1.5$（蓝色）、$2.5$（红色）和 $4.0$（绿色）三种归一化偏置条件。随着偏置电流的增加，可以观察到以下关键特征：第一，弛豫振荡频率$f_R$向高频移动（从约 2 GHz提升至4 GHz以上），这是因为更高的光子密度加快了载流子 - 光子相互作用的动力学过程；第二，共振峰的幅度逐渐降低，表明阻尼效应增强；第三，-3 dB 带宽$f_{-3\mathrm{dB}}$显著扩展，这意味着可以通过提高偏置电流来提升调制速度。然而，实际设计中还需要权衡功耗、热效应和非线性失真等因素，通常最优工作点在$I/I_{th}=2$--$3$之间。图中的箭头清晰标示了随电流增加响应曲线的演化趋势。
