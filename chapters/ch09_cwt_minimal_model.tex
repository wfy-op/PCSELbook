...
\]
再乘以纵向重叠积分（光子晶体层通常贡献约 10--20\% 的纵向模能量），有效耦合系数 $\kappa_1$ 的典型量级约为 $\kappa_1/(\omega/c) \sim 10^{-2}$--$10^{-1}$，对应波矢修正 $\delta k \sim |\kappa_1|/v_g\sim1$--$10\,\mathrm{cm}^{-1}$。这个量级与实验测量的 PCSEL 带隙宽度吻合，可作为初步设计的数量级参考。
\end{ExampleBox}
...
\end{equation}
其中 $\delta=\beta-\beta_0$ 是偏离 Bragg 条件的频率偏移，$\alpha$ 是模增益/损耗。关键在于耦合矩阵 $\bm{C}$ 自然分解为三个物理来源明确的子矩阵：
\begin{equation}
...
则界面反射引入的修正通常在 $\bm C_{\mathrm{rad}}$ 的 $5\%$ 以内，
...
\begin{equation}
\mu^{(r,s)} = -\frac{k_0^4}{2\beta}\,\xi_{p,q}\,\xi_{-r,-s}\int_{\mathrm{PC}}\!\!\int_{\mathrm{PC}} G(z,z')\phi_0(z')\phi_0(z)\,\dd z'\,\dd z,
...
求解 \cref{eq:3d_cwt_eigenvalue_full} 的本征值 $(\delta+\ii\alpha)$ 和本征向量 $\bm{V}$，可同时得到：
\begin{enumerate}
  \item \textbf{模式频率}：由 $\mathrm{Re}(\delta)$ 给出，对应 A/B/C/D 四模的频率排序；
  \item \textbf{辐射常数}：由 $\mathrm{Im}(\delta)$ 给出，即 $\alpha_{\mathrm{rad}}=2\,\mathrm{Im}(\tilde{\omega})$，直接量化面发射损耗；
  \item \textbf{面内场型与偏振}：由本征向量 $\bm{V}$ 给出，可判断模式的 irrep 归属和远场偏振；
...
\begin{equation}
\alpha_{\mathrm{rad}} = 2\,\mathrm{Im}\bigl[\lambda(\bm{C}_{\mathrm{rad}})\bigr] \propto \left|\int_{\mathrm{PC}} \xi_{1,0}(z)\,G(z,z')\,\xi_{-1,0}(z')\,\phi_0(z)\phi_0(z')\,\dd z\,\dd z'\right|^2.
...
\alpha_{\mathrm{rad}}
\propto
-2\,\mathrm{Im}\,\delta\lambda_{\mathrm{rad}}
\propto
|a_{\mathrm{rad}}|^2.
\]
...
从 \cref{eq:3d_cwt_eigenvalue_full} 的本征值可直接提取器件最关心的两个量。设第 $j$ 个本征值为 $\delta_j+\ii\alpha_j$，则对应的复频率为
\begin{equation}
\tilde{\omega}_j = \frac{(\beta_0+\delta_j)c}{n_{\mathrm{eff}}},
\label{eq:3d_cwt_complex_freq}
\end{equation}
其中 $n_{\mathrm{eff}}$ 是等效折射率。品质因子由复频率的实部与虚部之比给出：
\begin{equation}
...
\begin{equation}
\alpha_{\mathrm{rad},j} = 2\,\mathrm{Im}(\delta_j).
\label{eq:3d_cwt_alpha_rad}
\end{equation}
这个定义的优势在于它不依赖等效折射率的模糊定义——$\delta_j$ 直接来自本征值，可与 3D FDTD 的衰减率提取结果做无歧义比较 \cite{liang2012square}。
...
该框架已被应用于三晶格 PCSEL 设计 \cite{wang2024}，其中耦合系数
