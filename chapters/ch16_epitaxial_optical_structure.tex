...
\paragraph{读完本章，你应能}
\begin{itemize}
  \item 理解外延层为何与平面内晶格并列，成为 PCSEL 的第一性器件变量。
...
\paragraph{贯穿案例 B0：先交付纵向光学接口}
从本章起，\cref{tab:benchmark_pair} 中的 B0 固定为 \SI{940}{nm} 附近的 GaAs-like 教学器件：上、下 SCH 各先取约 \SI{200}{nm}，有源区先放置 3--5 个 InGaAs 量子阱。这里的数字只用于建立透明的初始模型，并非 Noda--Yoshida 器件的外延复原。本章向下一章交付目标纵模的 $n_{\mathrm{eff}}$、量子阱重叠 $\Gamma_{\mathrm{opt}}$ 和损耗区重叠 $\Gamma_{\mathrm{loss}}$，而不止于一张层栈图；三者都应随层厚扫描并附带边界条件。

\section{从二维光子晶体到真实器件：建立完整纵向物理链}
...
这条纵向链最容易被误读成“层栈画法”。更准确的读法是：外延层把一个冷腔光学模翻译成一组器件接口量，关键量是模场与有源区、损耗区各自的重叠，而非层名本身。\cref{fig:ch16_epi_overlap_interface_map} 把这件事画成一张接口地图，后续章节中的量子阱增益、电注入和热回写都要沿着这张图继续展开。

\begin{figure}[htbp]
  \centering
  \input{figures/fig_ch16_epi_overlap_interface_map}
  \caption{外延层作为器件接口的图形化读法。平面内光子晶体给出冷腔候选模，但外延层通过纵向模场、量子阱重叠和损耗区重叠决定它能否成为真实器件模。}
  \label{fig:ch16_epi_overlap_interface_map}
\end{figure}

\section[SCH 与器件可行性]{为什么 PCSEL 几乎总要落到 SCH：从教科书结构到器件可行性}
...
因此，SCH 的器件作用不能归结为沿用传统边发射激光器模板。它把 \cref{ch:threshold} 的阈值条件落实到层栈中：模式既能在量子阱附近获得足够光学重叠，又不至于被表面与接触损耗吃掉。
为了帮助读者直观理解 PCSEL 的典型纵向外延层堆叠结构，\cref{fig:ch16_epitaxial_stack}展示了一个完整的 GaAs 基 PCSEL 外延层结构示意图。典型的 PCSEL 外延结构从下至上依次包括：GaAs 衬底、下 SCH 波导层（约 200 nm）、多量子阱有源区（3-5 个 InGaAs 阱）、上 SCH 波导层（约 200 nm）、光子晶体层、电流阻挡层和 p 型接触层。某些设计可能在底部包含 n 型掺杂层以降低串联电阻，但 PCSEL 的面内反馈机制使其通常不需要 DBR 反射镜——这与 VCSEL 的结构形成鲜明对比。图中清晰标注了各层的典型厚度范围、材料体系和掺杂类型，这些参数对于理解后续章节的电注入设计和热管理策略至关重要。

\input{figures/fig_ch16_epitaxial_stack}
...
对常见 III--V 体系，$Q_c$ 必须按平台给初值，而不是跨平台共用单个“标准值”。\cref{tab:ch16_qc_platform} 给出本书采用的分平台起始区间，均应理解为\textbf{建模初值}：真实器件分析时仍需结合温度、应变和组分做二次标定。

\begin{table}[htbp]
  \centering
  \footnotesize
  \caption{$Q_c$ 的平台化初值口径（仅作建模起点，不是跨平台通用常数；温度 $T=300\,\mathrm{K}$)。}
  \label{tab:ch16_qc_platform}
  \setlength{\tabcolsep}{4pt}
  \begin{tabularx}{\textwidth}{@{}p{0.23\textwidth}p{0.18\textwidth}p{0.16\textwidth}Y@{}}
    \toprule
    材料平台 & $Q_c$ 起始区间 & 来源类型 & 说明 \\
    \midrule
    GaAs/AlGaAs（近红外） & $0.60$--$0.70$ & \textbf{[文献值]} & 教材与器件文献常见区间；室温附近常见初值约 $0.67$\cite{coldren2012,noda2024pcselreview} \\
    InGaAsP/InP（1.3/1.55 \si{\micro\meter}） & $0.35$--$0.45$ & \textbf{[文献值]} & 需随组分和应变重新标定，不可直接外推到 GaAs 平台\cite{coldren2012,noda2024pcselreview} \\
    InGaN/AlGaN（蓝光/深紫外） & $0.60$--$0.80$ & \textbf{[工程经验]}起点 & 极化场与应变耦合更强，建议先取宽区间再做平台内拟合\cite{lu2025deepuvpcsel,noda2024pcselreview} \\
    \bottomrule
  \end{tabularx}
\end{table}

这里应记住 $Q_c$ 是\textbf{材料平台相关的分配系数}，无需背诵一个脱离材料体系的“标准值”。只要 $Q_c$ 变化，电子与空穴各自的限制强弱、泄漏倾向以及对阻挡层的需求都会一起改变。

这对 PCSEL 的影响非常直接。若 $\Delta E_c$ 太小，电子在高注入或高温下容易越过量子阱区泄漏到上包层或接触层；若 $\Delta E_v$ 太小，则空穴更难被有效限制在有源区，因为空穴有效质量更大、迁移率更低、注入路径也常更差。于是，一个看似“光学上模重叠不错”的外延层，只要能带台阶设计不合理，到了电注入时就会表现出电子泄漏、空穴堆积或有源区载流子不足。这也是“冷腔结论不能直接变成器件结论”的第一层原因。
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \node at (1.7,4.2) {势垒/限制层};
    \node at (4.8,4.2) {量子阱};
    \node at (8.5,4.2) {势垒/限制层};
...
对于 PCSEL，这些机制之所以必须提前讲，是因为它们最终会把 \cref{ch:qw-gain} 的材料增益和 \cref{ch:electrical} 的注入分布改写成一个横向、纵向都不均匀的器件工作点。前面章节的冷腔模式排序，到了这里就开始遇到真正的器件反例：即便高 $Q$、高重叠或远场理想，也不自动对应低阈值器件\RedlineRef{2}。
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[x=1cm,y=0.85cm]
    ...
    \node[left] at (0,0.4) {衬底};
    \node[left] at (0,1.3) {n包层};
    \node[left] at (0,2.18) {下限制层};
    \node[left] at (0,2.8) {多量子阱};
    \node[left] at (0,3.45) {上限制层};
    \node[left] at (0,4.15) {光子晶体层};
    \node[left] at (0,4.88) {p包层/接触层};
  ...
  \caption{典型 PCSEL 外延层纵向结构示意。读图重点是每一层如何共同参与光学约束、载流子限制和损耗分布。}
  \label{fig:epi-stack}
\end{figure}
...
从求解步骤看，\cref{eq:slab_waveguide} 的逻辑非常朴素：在“芯层折射率高于包层”的区域，若 $\betaslab<nk_0$，解是振荡函数；在包层中若 $\betaslab>nk_0$，解会变成倏逝衰减。某一支模式由各层解在界面处的连续拼接共同选出，同时最外侧只允许衰减而非发散；单独观察某一层的解无法完成选模。因此，纵向模的身份从一开始就由整个层栈共同决定。
...
\end{equation}
这条式子虽然只是最简模型，却非常适合训练直觉：一旦芯层厚度、折射率对比或工作频率改变，$\betaslab$ 和纵向尾部衰减长度就会一起变化，这也是 PCSEL 模场被拉向量子阱、表面或接触层的根源。
\end{ExampleBox}
...
\cref{eq:gamma_qw} 可以从“局域材料增益如何变成模增益”直接推出。若量子阱区存在近似均匀材料增益 $g_{\mathrm{mat}}$，则某一纵向模的功率增长率可写成
\begin{equation}
g_{\mathrm{mod}}
=
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \node[left] at (0,2.95) {$p$ 型接触层};
    \node[left] at (0,1.45) {多量子阱};
...
\begin{notebox}
\textbf{自由载流子吸收的波长和温度依赖性。}式\cref{eq:fca_simple}中的系数$\sigma_n,\sigma_p$并非常数，而是强烈依赖于波长和温度。对GaAs 材料，经验公式给出~\cite{soref1987free}:
\[
\sigma_{n,p}(\lambda,T) \propto \lambda^q \cdot \left(\frac{T}{300\,\mathrm{K}}\right)^s,
\]
其中$q \approx 2$--$3$（波长指数），$s \approx 1$--$2$（温度指数）。这意味着:
\begin{itemize}
  \item 长波长器件（如 1.55 $\mu$m InP 基 PCSEL）的 FCA 损耗显著高于近红外器件
  \item 高温工作时 FCA 会进一步增加，形成正反馈的热 - 光耦合效应
  \item 在粗略估算中可取$\sigma_n \approx \SI{3e-18}{\centi\meter^2}$, $\sigma_p \approx \SI{7e-18}{\centi\meter^2}$（850 nm, 300 K）作为参考值
\end{itemize}
因此，在进行多波长或多温度仿真时，必须更新$\sigma_{n,p}$的值，而不能简单套用室温近红外的数据。
\end{notebox}
...
\section{层厚灵敏度与孔径工艺对比：从可算到可制造}
到器件设计阶段，读者最常问的是“哪一层最值得优先优化”。对这个问题，最有效的回答是一张一阶灵敏度表。\cref{tab:ch16_thickness_sensitivity} 给出本书 benchmark 层栈的教学型扫参示例（围绕基准值做 $\pm10\%$ 改动，观察关键接口量变化）。它的价值在于排序优先级，而不是提供可直接外推到所有平台的“万能百分比”。

\begin{table}[htbp]
  \centering
  \footnotesize
  \caption{benchmark 外延层栈的一阶灵敏度示例（参数扰动 $\pm10\%$）。数值用于设计优先级判断，不代表所有材料平台的统一结论。}
  \label{tab:ch16_thickness_sensitivity}
  \setlength{\tabcolsep}{4pt}
  \begin{tabularx}{\textwidth}{@{}p{0.22\textwidth}p{0.20\textwidth}Y@{}}
    \toprule
    扰动参数 & 关键量变化（代表趋势） & 设计含义 \\
    \midrule
    上波导厚度 $t_{\mathrm{core}}$ &
    $\Gamma_{\mathrm{opt}}$ 约变化 $\mp(4\%\text{--}7\%)$；$\eta_{u/d}$ 约变化 $\pm(5\%\text{--}8\%)$ &
    直接改写量子阱重叠与上下出光平衡，是优先级最高的纵向调参量之一。 \\
    光子晶体刻蚀深度 $h_{\mathrm{etch}}$ &
    $\alpha_{\mathrm{rad}}$ 常变化 $\pm(10\%\text{--}20\%)$；候选模排序对深刻蚀更敏感 &
    主要影响辐射通道与模式分裂，过深刻蚀常带来辐射损耗和工艺粗糙度双重代价。 \\
    p 包层厚度 $t_{p\mathrm{-clad}}$ &
    与损耗层重叠变化可导致 $\alpha_{\mathrm{int}}$ 约变化 $\pm(6\%\text{--}12\%)$ &
    与接触层、掺杂层协同设计；只看电阻不看光学损耗容易把阈值推高。 \\
    MQW 中心位置偏移 $\Delta z_{\mathrm{QW}}$（$\pm\SI{10}{nm}$） &
    $\Gamma_{\mathrm{opt}}$ 常变化约 $\pm(3\%\text{--}6\%)$ &
    在高 $Q$、窄窗口设计中会直接改写净模增益排序，需与生长公差联动评估。 \\
    \bottomrule
  \end{tabularx}
\end{table}

除了层厚，电流孔径工艺选择也会显著影响阈值与模式稳定窗口。以 VCSEL 体系的系统比较为例，指数引导氧化限制结构通常可获得更低阈值电流；而质子注入方案及增益引导氧化结构在横模选择性上可表现更优，同时具备更简化的工艺路径\cite{sarzala2010oxideproton}。这一权衡逻辑在 PCSEL 的电流孔径与注入工程中同样成立。

\input{figures/fig_ch16_oxide_vs_proton.tex}

\section{本章输出给后续器件章节的量}
到本章结束时，读者不应只得到一张层栈示意图，而应得到后面三章可直接调用的一组器件变量：目标模的纵向剖面、$\Gamma_{\mathrm{opt}}$、$\Gamma_{\mathrm{loss}}$、上下辐射不对称趋势，以及对层厚、折射率和异质结台阶变化的灵敏度。\cref{ch:qw-gain} 会把这些量与量子阱增益连接起来，\cref{ch:electrical} 会说明为什么这些外延层又决定了电流如何进入有源区，\cref{ch:eto-coupling} 则会展示温升如何反过来改写这些光学接口量。
...
\section*{练习题}
\begin{enumerate}
  \item \basicq 解释为什么 SCH 在 PCSEL 中是连接平面内光学设计与真实器件约束的基础，而非可省略的传统激光器结构。
...
\section*{延伸阅读}
\begin{itemize}
  \item 下一章将把本章建立的异质结、量子阱和重叠语言继续推进到有源区工程，解释单量子阱、多量子阱、应变和阻挡层为什么会改写材料增益到模增益的映射。 阅读时应同时核对本章变量在后续模型中的定义和适用范围。

  \item 若想对照一般半导体激光器中的 SCH 与波导理论，可参考 \cite{coldren2012} 中关于异质结波导、约束因子和损耗重叠的章节，但阅读时应始终记得：PCSEL 还要额外面对开放辐射与面内反馈耦合。 阅读时应重点核对相关模型假设、参数范围与文献适用条件。

  \item 若想从器件实例反看外延层如何进入 PCSEL 主线，可参考 \cite{imada1999,hirose2014,zhoupan2023}。 阅读时应重点核对相关模型假设、参数范围与文献适用条件。
\end{itemize}
