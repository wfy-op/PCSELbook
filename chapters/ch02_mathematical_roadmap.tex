...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_ch02_knowledge_map.tex}
  \caption{全书知识依赖关系思维导图。根节点为 Maxwell方程+Bloch定理两大理论基础；第一层分支包括能带理论、反馈机制、阈值条件三大核心模块；第二层涵盖 CWT、BIC、CMT 等进阶工具；第三层延伸至器件建模、动态学、热 - 电 - 光耦合等应用场景。每章标注了难度等级（★为基础，★★★为进阶）和预计学习时间，帮助学生规划阅读路径。}
  \label{fig:ch02_knowledge_map}
\end{figure}

\cref{fig:ch02_knowledge_map}给出了全书的知识依赖关系图，帮助学生建立整体认知框架并合理规划学习路径。
...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_ch02_benchmark_device}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[node distance=1.25cm and 1.6cm,>=Latex,
    ...
    \node[block] (a) {第3--5章\\麦克斯韦方程, 布洛赫, 能带};
    \node[block,below=of a] (b) {第6--10章\\面发射, 阈值, 偏振/连续谱束缚态, 耦合波};
    \node[block,below=of b] (c) {第11--16章\\耦合模/平面波展开/严格耦合波/时域差分/有限元/互证};
...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_ch02_six_redlines}
