...
\paragraph{两类贯穿案例及证据边界}
为了让后半本的器件章节持续追踪同一组对象，本书把案例分成两个层次。B0 是可计算的教学基准，用于演示从外延到热回写的接口；R1 是有文献依据的性能锚点，用于检验放大后的设计逻辑。两者不能混写：B0 的参数带有\textbf{[本书算例]}标记，不代表已发表器件的完整外延配方；R1 只引用论文明确报告的尺寸和性能，不用缺失的层厚、电流或温升数据补齐故事。

\begin{table}[htbp]
  \centering
  \small
  \caption{贯穿全书的双层案例。B0 提供透明的计算输入，R1 提供可追溯的实验性能边界。}
  \label{tab:benchmark_pair}
  \setlength{\tabcolsep}{4pt}
  \begin{tabularx}{\textwidth}{@{}p{1.0cm}p{3.2cm}YY@{}}
    \toprule
    代号 & 身份与尺度 & 本书采用的数据 & 在器件章节中的用途 \\
    \midrule
    B0 & \textbf{[本书算例]} GaAs-like 方格圆孔 PCSEL，目标波长 \SI{940}{nm} & 上、下 SCH 各约 \SI{200}{nm}，3--5 个 InGaAs 量子阱；热学入门估算先用直径 \SI{200}{\micro\meter} 的热源。具体组分、掺杂和接触参数必须在真实项目中重新标定 & \cref{ch:epitaxial-optics,ch:qw-gain,ch:electrical,ch:eto-coupling} 依次交接 $n_{\mathrm{eff}}$、重叠因子、局域增益、$J(x,y)$ 与 $T(x,y,z)$ \\
    R1 & \textbf{[文献值]} Noda--Yoshida 双晶格放大路线 & \SI{500}{\micro\meter} 级器件达到约 \SI{10}{W} 脉冲输出；\SI{1}{mm} 级器件实现 \SI{10}{W} 连续输出与低于 \SI{1.23}{kHz} 的内禀线宽；\SI{3}{mm} 级器件实现 \SI{50}{W} 连续输出和约 \ang{0.05} 发散角\cite{yoshida2019,yoshida2023,noda2023tutorial} & \cref{ch:single-mode-design,ch:metrics} 用这些公开指标检验模鉴别、热补偿和性能报告是否保持在证据边界内 \\
    \bottomrule
  \end{tabularx}
\end{table}

这并非说全书结论只对这一类器件成立，而是说：\textbf{若某章没有特别声明，我们优先把抽象公式和分类工具投影回这一个统一对象。} 这样做的好处是，读者从 \cref{ch:bloch,ch:gamma,ch:pwe} 读到 \cref{ch:workflow,ch:worked-example} 时，脑中追踪的是同一个器件在不同层级的投影，而不是每章换一个新结构。
...
\begin{RigorousBox}
一旦进入开放系统，本征频率通常是复数。复频率虚部是模式辐射或损耗的时间尺度编码，而非“数值噪声的副产物”。若把开放系统错当成只有实频率的封闭系统，就会直接丢失辐射损耗信息。
\end{RigorousBox}
...
仅知道“后面有哪些章节”还不够，还需要知道每一段向下一段交付什么中间量。\cref{tab:chapter-handoff} 给出本书最关键的章节交接关系。
