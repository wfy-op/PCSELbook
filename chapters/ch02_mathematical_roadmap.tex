...
本章不推导新的器件结果，而是固定全书使用的共同语言。先统一坐标和频域约定，再建立晶格与倒格矢记号，随后说明模型层级与术语边界，最后给出阅读全书时必须守住的几条“红线”。
\begin{figure}[htbp]
  \centering
...
\section*{建议继续阅读}
\begin{itemize}
  \item \cite{sakoda2005} 中关于晶格、倒格矢与对称性记号的章节。 建议重点对照其中的符号、层级和模型接口。

  \item 建议将本书的符号表与本章一起对照阅读，随时检查各符号所属的模型层级。 建议重点关注本章输出量如何进入后续模型。
\end{itemize}
