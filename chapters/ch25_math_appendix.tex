...
\paragraph{阅读主线}
\ChapterModelLevel{跨层数学支撑（L1→L4）}
\input{figures/fig_ch25_math_object_map}
图 \cref{fig:ch25_math_object_map} 先把本章数学对象之间的关系固定下来，再进入各节公式推导，可明显降低“符号会看、结构不清”的阅读负担。

\paragraph{首次阅读建议路径}
若是第一次阅读本附录，建议按以下顺序进行：先读时间因子与傅里叶约定，再读周期/Bloch 展开与卷积结构，再读 QNM 与归一化，最后读互易与微扰公式。其余段落可在需要时回查。

本附录是把正文反复调用、却不宜每次都从头展开的数学骨架集中讲清楚，而非把若干常见恒等式堆在一起的公式仓库。我们先统一时间因子和频域约定，因为一切损耗、增益和复频率的符号都依赖它；随后建立周期介质中的傅里叶与 Bloch 展开，解释卷积结构为什么在 \cref{ch:gamma} 的能带概念、\cref{ch:pwe} 的 PWEM、以及后续 RCWA 和 CWT 中一再出现；再讨论内积、归一化和开放系统中的复频率与 $Q$；最后给出互易性与一阶微扰公式，并明确说明它们在哪些条件下可靠、在哪些条件下只能作为工程近似。

\section{为什么数学附录不是装饰，而是主结论的一部分}
PCSEL 的建模链横跨电磁场、半导体输运和热传导。只要这些模块之间的时间因子、损耗符号、傅里叶约定或归一化不一致，最终就会出现一种很危险的情况：看起来像是“物理结论不同”，实际却只是数学约定变了。最典型的例子就是时间因子的选择。一旦一个章节使用 $\ee^{-\ii\omega t}$，另一个局部推导却默默改成 $\ee^{+\ii\omega t}$，那么介质虚部、损耗符号、复频率虚部和 $Q$ 的定义就会全部翻转。若读者没有意识到这种符号翻转，很容易把纯符号问题误读成腔损耗差异或增益符号差异。

因此，本附录的目标是把正文各章默认共享的数学地基显式写出来，而非补充“额外数学”。只要地基统一，后面的开放系统、耦合波理论、RCWA、FDTD/FEM 提取和器件级回写才可能真正接成一条链。

\begin{IntuitionBox}
如果把整本书看成一套多物理场模型，那么数学附录的作用就像接口协议。接口协议一旦不统一，上层结果可能只是数值上偶然一致，而不能视为可复用结论。
\end{IntuitionBox}
...
在封闭、无损、厄米问题中，我们习惯把本征模视为某个 Hilbert 空间里的正交基。这个直觉很有用，但它并不能原封不动地搬到 PCSEL 的开放模问题上。根源在于开放系统的共振模满足的是\textbf{纯出射边界条件}，而不是封闭边界条件。对一个复频率模
\begin{equation}
...
\end{equation}
模场若按纯出射条件延拓到无穷远，空间上通常不是衰减而是增长。这并非说模式“物理发散”，而是说它不再属于普通平方可积的束缚态函数空间。开放模更自然的数学身份，是 Green 张量或散射矩阵在复频率平面上的极点态 \cite{leung1994complete,alpeggiani2017qnm}。
...
\end{equation}
只要 $|r_1r_2|<1$，其解 $\tilde{k}$ 必为复数，等价地 $\tilde{\omega}=c\tilde{k}$ 也必为复数。这个最小例子非常值得记住，因为它说明：\textbf{复频率是开放边界条件本身逼出来的本征值，而非后来为了描述损耗而“补上去”的修正。}
\end{ExampleBox}
\subsection{QNM 理论的三个核心数学问题（进阶选读）}
准正规模理论在实际应用中面临三个相互关联的数学挑战：(i) 模场的空间增长导致传统$L^2$内积发散；(ii) 非厄米算符的本征模不构成正交系而需采用双正交基；(iii) QNM集合的完备性问题。本节逐一解决这些问题。

\subsubsection*{1. 发散积分的正则化处理}

考虑一维 Fabry-Perot 腔的 QNM解。在腔外区域($|x|>L/2$)，模场具有如下渐近形式:
\begin{equation}
E_n(x) \sim E_0 \ee^{\ii \tilde{k}_n |x|}, \qquad \tilde{k}_n = \frac{\tilde{\omega}_n}{c} = k_n - \ii \kappa_n,
\label{eq:qnm_spatial_growth_detail}
\end{equation}
其中$\kappa_n > 0$对应模式的辐射衰减率。将复波矢代入模场表达式可得，当$x\to\infty$时，$|E_n(x)|^2 \sim \ee^{2\kappa_n x}$指数增长，导致传统的$L^2$范数积分发散:
\begin{equation}
\int_{-\infty}^{\infty} |E_n(x)|^2 \dd x = \infty.
\label{eq:l2_divergence}
\end{equation}

为解决这一问题，现代 QNM理论采用以下几种等价的正则化方案:

\textbf{(a) 体积极限正则化 (Volume regularization)}: 先将系统限制在有限体积$V_L=[-L,L]$内计算，最后取$L\to\infty$极限并提取有限部分(finite part)。具体而言，将发散积分写为:
\begin{equation}
\int_{-L}^{L} |E_n(x)|^2 \dd x = A \ee^{2\kappa_n L} + B(L),
\end{equation}
其中$A$是发散的系数，$B(L)$是随$L$缓变的部分。定义正则化后的内积为去掉指数发散项后的剩余部分:
\begin{equation}
\langle E_n, E_m \rangle_{\rm reg} := \lim_{L\to\infty} \left[ \int_{-L}^{L} E_n^*(x) E_m(x) \dd x - \delta_{nm} \frac{A_n}{2\kappa_n}\ee^{2\kappa_n L} \right].
\label{eq:volume_regularization}
\end{equation}

\textbf{(b) 解析延拓法 (Analytic continuation)}: 将 QNM视为复波数平面上的亚纯函数，通过将实轴积分路径旋转到复平面的适当位置来实现收敛。设$k = \tilde{k}_n + s \ee^{\ii\theta}$，选择旋转角$\theta$使得被积函数沿新路径指数衰减。这种方法与留数定理自然衔接。

\textbf{(c) PML 正则化}: 引入完美匹配层坐标变换$x \to \tilde{x}(x) = x + \ii f(x)$，其中$f(x)$在计算域外部单调递增。在此变换下，原发散的被积函数变为:
\begin{equation}
\int_{-\infty}^{\infty} |E_n(x)|^2 \dd x \quad \longrightarrow \quad \int_{-\infty}^{\infty} |E_n(\tilde{x})|^2 \frac{\dd\tilde{x}}{\dd x} \dd x,
\end{equation}
由于$\Im[\tilde{k}_n \tilde{x}] < 0$在外区成立，积分自动收敛。这是商业软件中最常用的实现方式。

这三种方法在数学上是等价的，都给出了相同的广义归一化结果~\cite{kristensen2015qnm,sauvan2013qnm}。

\subsubsection*{2. 双正交基的严格构造}

对于非厄米 Maxwell 算符$\hat{\Theta}$，其右本征模$\{|R_n\rangle\}$满足:
\begin{equation}
\hat{\Theta}|R_n\rangle = \tilde{\omega}_n^2 |R_n\rangle,
\end{equation}
但这些右模之间并不正交:$\langle R_m|R_n\rangle \neq 0$ ($m\neq n$)。为了构建有效的模态展开，必须引入伴随算符$\hat{\Theta}^\dagger$的左本征模$\{|L_n\rangle\}$:
\begin{equation}
\hat{\Theta}^\dagger|L_n\rangle = (\tilde{\omega}_n^2)^* |L_n\rangle.
\label{eq:left_eigenmode_def}
\end{equation}

关键结论是：左右模构成\textbf{双正交系}(biorthogonal system):
\begin{equation}
\langle L_m|R_n\rangle = N_n \delta_{mn},
\label{eq:biorthogonality_relation}
\end{equation}
其中$N_n$是依赖于归一化约定的常数。这里的内积应采用前述正则化方案之一来计算。

\paragraph{双正交系的几何图像} 在非厄米系统中，左模和右模不再是彼此的共轭转置，而是两个独立的向量集。右模$\{|R_n\rangle\}$张成状态空间，而左模$\{|L_n\rangle\}$作为其对偶基底(dual basis)。任意物理态$|\Psi\rangle$按右模展开时，展开系数由对应的左模投影给出:$c_n=\langle L_n|\Psi\rangle/N_n$。这种关系类似于协变矢量和逆变矢量——通过配对缩并得到标量。

\subsubsection*{3. 完备性关系与模态展开}

若 QNM集合$\{|R_n\rangle\}$是完备的，则任意物理态$|\Psi(t)\rangle$可以展开为:
\begin{equation}
|\Psi(t)\rangle = \sum_n c_n(t) |R_n\rangle, \qquad c_n(t) = \frac{\langle L_n|\Psi(t)\rangle}{\langle L_n|R_n\rangle}.
\label{eq:qnm_complete_expansion}
\end{equation}
完备性的数学表述是闭包关系(closure relation):
\begin{equation}
\sum_n \frac{|R_n\rangle\langle L_n|}{\langle L_n|R_n\rangle} = \hat{I},
\label{eq:closure_relation}
\end{equation}
其中$\hat{I}$是单位算符。

然而，对于开放系统，QNM的完备性并非总是成立。Leung等人~\cite{leung1994complete}证明了以下重要结论:

\begin{itemize}
  \item \textbf{有限尺寸散射体}: 若介电常数扰动$\Delta\varepsilon(\rvec)$具有紧支集 (compact support)，则 QNM集合在散射体外部的区域内是完备的。
  
  \item \textbf{色散介质}: 当$\varepsilon(\omega)$显含频率时，需要在扩展的状态空间（包含介质极化自由度）中讨论完备性。此时 QNM可能与连续谱混合，形成所谓的"quasinormal continuum"。
  
  \item \textbf{高 Q 值近似}: 对于高品质因子腔($Q\gg1$)，前几个主导 QNM通常足以精确描述腔内的场分布，即使数学上不完备，工程上也足够准确。
\end{itemize}

从 Green 函数的角度看，完备性等价于说 Green 算符可以用 QNM展开:
\begin{equation}
\overleftrightarrow{G}(\rvec,\rvec';\omega) = \sum_n \frac{\E_n(\rvec)\otimes\E_n^{\rm L}(\rvec')}{2\tilde{\omega}_n(\omega-\tilde{\omega}_n)},
\label{eq:green_qnm_expansion_detailed}
\end{equation}
其中$\E_n^{\rm L}$是左模的电场分量。这个展开式在远离分支切割(branch cut)的区域一致收敛，而在接近连续谱阈值时需要额外小心处理~\cite{sauvan2013qnm,kristensen2015qnm}。

\paragraph{实际应用建议} 对于 PCSEL仿真，推荐采用以下工作流程:
\begin{enumerate}
  \item 使用 COMSOL/Lumerical 的 QNM求解器获取前$N$个主导模式的复频率$\tilde{\omega}_n$和场分布$\E_n(\rvec)$
  \item 在同一平台上计算对应的左模$\E_n^{\rm L}$(或通过互易性关系间接获得)
  \item 验证模态截断误差：先计算双正交投影系数
  \[
  c_n=\frac{\langle L_n|\Psi_{\rm target}\rangle}{\langle L_n|R_n\rangle},
  \]
  再以重构残差定义误差
  \[
  \eta=
  \frac{\left\|\Psi_{\rm target}-\sum_{n=1}^{N} c_n R_n\right\|_{W}}
  {\|\Psi_{\rm target}\|_{W}}
  \]
  \item 若$\eta < 1\%$,则认为当前 QNM基组足够用于后续的微扰分析和耦合计算
\end{enumerate}
...
\begin{equation}
F_P(\rvec_0,\hat{\bm p},\omega)
\approx
\sum_n \Re\!\left[\frac{A_n}{\omega-\tilde{\omega}_n}\right],
...
A_n=
\frac{6\pi c}{\omega}\,
\frac{\bigl(\hat{\bm p}\!\cdot\!\E_n^{\mathrm R}(\rvec_0)\bigr)\bigl(\hat{\bm p}\!\cdot\!\E_n^{\mathrm L}(\rvec_0)\bigr)}
{2\tilde{\omega}_n\langle \E_n^{\mathrm L},\E_n^{\mathrm R}\rangle}
\]
是由左右模重叠与归一化共同决定的复残数。仅在单模、弱重叠、高 $Q$ 近似下，\cref{eq:purcell_modal_sum} 才可进一步退化为 Lorentzian 峰形；一般开放系统可出现非 Lorentz 线形和模间干涉项。
...
互易性并非无条件成立。若介质存在显著非互易效应、时变调制、磁光非对称张量或非局域响应，则\cref{eq:lorentz_reciprocity_appendix} 需要修改，甚至可能失效。将其直接套用到所有有源介质问题上是不严谨的。
...
若扰动是附加小吸收 $\delta\varepsilon''>0$，则同样的骨架会给出复频率虚部修正，也就是额外损耗，而非实部折射率。于是，从数学上看，“热漂移改频率”和“掺杂吸收改线宽”其实是同一套开放系统微扰公式的不同投影：前者更看重 $\Re\,\delta\tilde{\omega}$，后者更看重 $\Im\,\delta\tilde{\omega}$。
...
\section{从 CWT 本征矢量提取远场偏振：方位角、椭圆率与 Stokes 参量}
\label{sec:cwt-polarization-extraction}
本节给出一组可直接写进代码的公式：如何从 \cref{ch:cwt-min,ch:cwt-3d} 的四波 CWT 本征矢量读出远场偏振态。
...
2\,\mathrm{Re}\!\left[c_{x,j}c_{y,j}^{\ast}\right],\;
|c_{x,j}|^2-|c_{y,j}|^2
\right),
\qquad
\psi_j \in \left(-\frac{\pi}{2},\frac{\pi}{2}\right].
\label{eq:polarization_azimuth_from_cxy}
\end{equation}
特别地，当相位差接近零时，\cref{eq:polarization_azimuth_from_cxy} 会退化到熟悉的幅值比公式；当 $|c_{x,j}|=|c_{y,j}|$ 且相位差为零时，$\psi_j=45^\circ$，对应对角线偏振；当 $c_{y,j}=0$ 时，$\psi_j=0^\circ$，对应纯 $x$ 偏振。

\paragraph{2. 椭圆率 (ellipticity)}
若考虑复振幅的相对相位，先定义椭圆角 $\chi_j$：
\begin{equation}
\chi_j
=
\frac{1}{2}\arcsin\!\left(
\frac{2\,\mathrm{Im}\!\left[c_{x,j}^{\ast}c_{y,j}\right]}{|c_{x,j}|^2 + |c_{y,j}|^2}
\right),
\qquad
e_j = \tan\chi_j,
\qquad
...
\section{群论工具箱：\texorpdfstring{$C_{4v}$}{C4v} 点群完整参考}
\label{sec:group-theory-toolbox}
本节把 \cref{ch:polarization} 只按需引用的群论事实集中成一份完整参考：生成元、特征标表、直积表、选择规则与 $C_{4v}\to C_{2v}$ 分支律。它服务于查阅而非线性阅读。

\textbf{生成元与群元素}：$C_{4v}$ 由两个生成元生成：
\begin{itemize}
  \item $C_4$: 绕 $z$ 轴旋转 $90^\circ$（逆时针方向为正）
  \item $\sigma_v$: 关于 $xz$ 平面（即 $y=0$ 平面）的镜面对称
\end{itemize}
全部 8 个群元素为：$\{E, C_4, C_4^2=C_2, C_4^3, \sigma_{v,x}, \sigma_{v,y}, \sigma_{d,xy}, \sigma_{d,-xy}\}$。其中：
\begin{itemize}
  \item $E$: 恒等操作
  \item $C_4, C_4^3$: $\pm 90^\circ$ 旋转（这两者属于同一类）
  \item $C_2=C_4^2$: $180^\circ$ 旋转
  \item $\sigma_{v,x}, \sigma_{v,y}$: 垂直镜面（分别关于 $yz$ 和 $xz$ 平面）
  \item $\sigma_{d,xy}, \sigma_{d,-xy}$: 对角镜面（分别关于 $y=x$ 和 $y=-x$ 平面）
\end{itemize}
...
表中每一行的物理含义：当对该 irrep 的基函数施加相应的对称操作时，函数乘以对应的特征标值（$+1$ 保持不变，$-1$ 改变符号）。例如，$A_1$ 表示在所有 8 个操作下都不变——这是"完全对称"的表示；而 $E$ 是二维表示，其特征标为 0 意味着两个基函数在该操作下互相交换或线性组合。

\textbf{PCSEL 中的模式分类实操步骤}：
\begin{enumerate}
  \item \textbf{提取近场分布}: 从仿真中获得 $|\mathbf{E}(x,y)|^2$ 或特定分量的实部/虚部
  \item \textbf{应用对称操作}: 对场分布依次施加 $C_4$ 旋转和四个镜面反射
  \item \textbf{记录响应}: 每次操作后，判断场是 (a) 不变 (+1)、(b) 反相 (-1)、还是 (c) 变成另一个正交模式（二维表示的特征）
  \item \textbf{查表比对}: 将得到的特征标序列与特征标表逐行对比，确定归属
  
  \textit{快捷技巧}：实际上只需要测试三个关键操作即可唯一确定大多数情况：$C_4$（区分单重态/双重态）、$\sigma_v$（区分 $A/B$ 型）、$\sigma_d$（区分下标 1/2）。
\end{enumerate}

基于上述流程，PCSEL 模式可分为两类：
\begin{itemize}
  \item \textbf{单重态} ($A_1, A_2, B_1, B_2$): 非简并模式，电场具有确定的宇称变换性质。这些模式往往是 BIC 候选（若不与辐射通道 irrep 匹配）。
  \item \textbf{双重态} ($E$): 二重简并模式，对应一对正交偏振态。在无扰动情况下能量严格相等，是典型的亮模候选。
\end{itemize}

\textbf{选择规则的群论表述}：考虑任意微扰算符 $\hat{O}$（可以是介电常数扰动、表面形变、电流注入等）诱导的两个模式 $|\psi_i\rangle$ 和 $|\psi_f\rangle$ 之间的耦合矩阵元：
\begin{equation}
M_{fi} = \langle\psi_f|\hat{O}|\psi_i\rangle.
\end{equation}
该矩阵元非零的\textbf{必要条件}是三者的直积包含全对称表示 $A_1$：
\begin{equation}
\Gamma_i \otimes \Gamma_O \otimes \Gamma_f \supset A_1,
\label{eq:selection_rule_general}
\end{equation}
其中 $\Gamma$ 表示所属的 irrep。利用 $C_{4v}$ 的直积表可以快速判断：
\begin{center}
\small
\begin{tabular}{c||cccc|c}
$\otimes$ & $A_1$ & $A_2$ & $B_1$ & $B_2$ & $E$ \\ \hline\hline
$A_1$ & $A_1$ & $A_2$ & $B_1$ & $B_2$ & $E$ \\
$A_2$ &       & $A_1$ & $B_2$ & $B_1$ & $E$ \\
$B_1$ &       &       & $A_1$ & $A_2$ & $E$ \\
$B_2$ &       &       &       & $A_1$ & $E$ \\
$E$   &       &       &       &       & $A_1\oplus A_2\oplus B_1\oplus B_2$ \\
\end{tabular}
\end{center}

\textbf{实例分析 1：四极矩辐射的选择规则}\\
考虑四极矩辐射算符的分量 $Q_{xy} \propto xy$。从特征标表可知 $xy$ 属于 $B_2$ 表示。现在判断几组可能的跃迁：
\begin{itemize}
  \item $A_1 \leftrightarrow B_2$ 跃迁：$A_1 \otimes B_2 \otimes B_2 = A_1 \otimes A_1 = A_1$ ✓ （对称性允许）
  \item $A_1 \leftrightarrow A_1$ 跃迁：$A_1 \otimes B_2 \otimes A_1 = B_2 \not\supset A_1$ ✗ （对称性禁戒）
  \item $B_1 \leftrightarrow E$ 跃迁：$B_1 \otimes B_2 \otimes E = A_2 \otimes E = E \not\supset A_1$ ✗ （禁戒）
\end{itemize}
这解释了为什么某些对称性保护的 BIC 无法向自由空间辐射——是因为对称性从根本上禁止了这种耦合，而非因为"损耗太小"。

\textbf{实例分析 2：法向辐射通道的 irrep}\\
在 $\Gamma$ 点，法向辐射对应的是横向均匀平面波，其两支线偏振 $(\hat{x},\hat{y})$ 构成 $E$ 表示的基。因此：
\begin{itemize}
  \item 若腔模属于 $E$ 表示，则 $\Gamma_{\rm mode} \otimes \Gamma_{\rm rad} = E \otimes E = A_1 \oplus A_2 \oplus B_1 \oplus B_2 \supset A_1$ ✓ —— 允许辐射（亮模）
  \item 若腔模属于 $A_1$ 表示，则 $A_1 \otimes E = E \not\supset A_1$ ✗ —— 禁止法向辐射（BIC 候选）
\end{itemize}
因此，高阶带边模式（通常是 $E$ 表示）可表面发射，而基模（通常是 $A_1$）可能成为暗模。

\textbf{对称性破缺与不可约表示分解}：当结构发生某种变形或引入各向异性时，对称性会降低。设原始对称群为 $G$，破缺后的子群为 $H \subset G$。此时 $G$ 的每个 irrep 按照一定规则"分解"为 $H$ 的若干个 irrep 的直和。这个过程称为\textbf{分支律}(branching rule)。

对于 PCSEL 常见的 $C_{4v} \to C_{2v}$ 破缺（例如引入椭圆形孔或矩形晶格），分解规则如下：
\begin{align}
...
\label{eq:c4v_to_c2v_decomposition}
\end{align}

关键的物理后果：
\begin{itemize}
  \item \textbf{双重态分裂}: $E$ 表示分解为两个不同的单重态 $B_1$ 和 $B_2$。这意味着原本简并的一对模式会发生能量劈裂，劈裂大小正比于对称性破缺强度。
  
  \item \textbf{偏振锁定}: 分裂后的两个模式分别继承原 $E$ 表示的两个正交基，对应固定的线偏振方向。这在实验中表现为输出光的偏振方向被"钉扎"在某个晶轴方向。
  
  \item \textbf{选择规则放宽}: 由于 $C_{2v}$ 的特征标表更简单（只有 4 个一维 irrep），原来在 $C_{4v}$ 中被禁戒的某些跃迁可能在 $C_{2v}$ 中变得允许。这可以用来解释为何轻微的结构不对称会导致 BIC 泄漏出有限的辐射。
\end{itemize}

\paragraph{实际应用建议} 在设计 PCSEL 时，可以主动利用对称性工程：
\begin{enumerate}
  \item 若要实现稳定的单线偏振输出，故意打破 $C_{4v}$ 对称性（如采用椭圆孔或矩形占空比）
  \item 若要追求最高品质因子的 BIC，保持完美的 $C_{4v}$ 并确保工作点是 $A_1/A_2/B_1/B_2$ 之一
  \item 若要实现双波长或双偏振激光器，可以利用接近简并但略微分裂的准-$E$ 态
\end{enumerate}
...
$\bm{C}_{\mathrm{1D}}$ & \cref{ch:cwt-3d}，\cref{eq:3d_cwt_C1D_matrix}：一维反馈耦合矩阵。 \\
$\bm{C}_{\mathrm{rad}}$ & \cref{ch:cwt-3d}，\cref{eq:3d_cwt_Crad_matrix}：辐射耦合矩阵，非厄米，来自开放通道。 \\
$\bm{C}_{\mathrm{2D}}$ & \cref{ch:cwt-3d}，\cref{eq:3d_cwt_C2D_matrix}：二维高阶耦合矩阵。 \\
$v_{\mathrm{en}}$ & \cref{ch:threshold}，\cref{eq:energy_velocity_def}：能量速度，将空间增益/损耗映射为时间增长率的参数。 \\
$\tilde n^2=n^2+2\ii k_0 n\tilde\alpha$ & \cref{ch:cwt-3d}，\cref{eq:3d_cwt_vector_wave}：含增益/损耗的复折射率。 \\
$\xi_{m,n}(z)$ & \cref{ch:cwt-3d}，\cref{eq:3d_cwt_n2_expand}：光子晶体层的 Fourier 系数。 \\
$\phi_0(z)$ & \cref{ch:cwt-3d}，\cref{eq:3d_cwt_fundamental_mode}：背景层状波导的基模纵向剖面。 \\
...
$G(z,z')$ & \cref{ch:cwt-3d}，\cref{eq:3d_cwt_green}：辐射波的纵向 Green 函数（近似局域均匀背景）。 \\
$G_{m,n}(z,z')$ & \cref{ch:cwt-3d}，\cref{eq:3d_cwt_green_ho}：高阶波在光子晶体层内的倏逝 Green 函数。 \\
$\delta=\beta-\beta_0$ & \cref{ch:cwt-3d}，\cref{eq:3d_cwt_eigenvalue}：偏离 Bragg 条件的传播常数偏移。 \\
$\alpha_{\mathrm{rad},j}$ & \cref{ch:cwt-3d}，\cref{eq:3d_cwt_alpha_rad}：第 $j$ 模的辐射常数，$\;2\,\mathrm{Im}(\lambda_j)$（亦可写为 $-2\,\mathrm{Im}(\tilde\omega_j)/v_{g,j}$）。 \\
...
$\Gamma_{\mathrm{PC}}$ & \cref{ch:pwe}，\cref{eq:gamma_pc}：光子晶体层的光学限制因子，用于二维等效介电常数修正。 \\
$\tilde\varepsilon_a,\;\tilde\varepsilon_b$ & \cref{ch:pwe}，\cref{eq:modified_eps}：送入二维 PWEM 的修正等效介电常数。 \\
$\alpha_{\mathrm{rad}}=2\,\mathrm{Im}(\lambda)=-2\,\mathrm{Im}(\tilde\omega)/v_g$ & \cref{ch:cwt-3d}，\cref{eq:3d_cwt_alpha_rad}：辐射常数与 CWT 本征值虚部（及复频率虚部）之间的换算关系。 \\
...
\section*{回看本章}
数学附录的作用是统一全书的语法，而非堆公式。时间因子决定损耗与增益的符号，傅里叶卷积解释了周期介质中的耦合矩阵，QNM / resonant states 说明开放模为何不是普通束缚态，左/右模与广义归一化说明为什么开放系统不能直接照搬厄米内积，复频率给出 $Q$ 与光子寿命的精确定义，互易性与一阶微扰则为耦合和灵敏度分析提供了共同骨架。只要这些约定统一，正文中的光学、器件和仿真章节就能共享同一套数学接口；若这些约定混乱，后面的一切“高级模型”都可能只是表面连贯。
...
\section*{延伸阅读}
\begin{itemize}
  \item 下一章将把这些数学语法压缩成器件级建模的符号链，特别是从材料增益到阈值电流的映射。 阅读时应同时核对本章变量在后续模型中的定义和适用范围。

  \item 若想进一步打牢开放系统与扰动理论，可对照 \cite{joannopoulos2008,sakoda2005} 中关于周期介质与模式扰动的附录阅读。 阅读时应重点核对开放系统归一化、QNM 与非厄米微扰的标准假设。

  \item 若想系统补强开放系统归一化、QNM 与非厄米微扰，可继续阅读 \cite{leung1994complete,leung1994perturb,muljarov2010bw,sauvan2013qnm,kristensen2015qnm,alpeggiani2017qnm}。 阅读时应重点核对开放系统归一化、QNM 与非厄米微扰的标准假设。
\end{itemize}
\begin{table}[htbp]
  \centering
