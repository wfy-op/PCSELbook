...
$\alpha_{\mathrm{rad},j}$ & \cref{ch:cwt-min}，\cref{eq:3d_cwt_alpha_rad}：第 $j$ 模的辐射常数，$\;2\,\mathrm{Im}(\lambda_j)$（亦可写为 $-2\,\mathrm{Im}(\tilde\omega_j)/v_{g,j}$）。 \\
...
$\alpha_{\mathrm{rad}}=2\,\mathrm{Im}(\lambda)=-2\,\mathrm{Im}(\tilde\omega)/v_g$ & \cref{ch:cwt-min}，\cref{eq:3d_cwt_alpha_rad}：辐射常数与 CWT 本征值虚部（及复频率虚部）之间的换算关系。 \\
