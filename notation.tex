...
\chapter*{证据标签规范}
\addcontentsline{toc}{chapter}{证据标签规范}

为避免把教学示意、工程经验和可核验文献值混写，本书统一使用下列证据标签：
...
\midrule
\endhead
\textbf{[文献值]} & 来自可核验的同行评审文献；引用时应给 DOI 或标准引文。 \\
\textbf{[本书算例]} & 来自本书示例脚本或示例工况；默认不可直接外推到其他平台。 \\
\textbf{[工程经验]} & 来自工程实践的启发式范围；必须同时给出边界条件。 \\
\textbf{[教学示意]} & 仅用于帮助理解机制与数量级，不作为发布级定量结论。 \\
\textbf{[预印本补充：待同行评审]} & 用于前沿趋势补充；若作为证据出现，正文必须同时给出“待同行评审”提示，并尽量配套同行评审兜底文献。 \\
\bottomrule
\end{longtable}
